...
__global__ void p041_lesson_kernel(const float* input, float* output, int count) {
  const int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < count) output[index] = input[index] * 41.0f + 1.0f;
}
...
int main() {
  ...
  p041_lesson_kernel<<<(count + 127) / 128, 128>>>(device_input, device_output, count);
  ...
  for (int i = 0; i < count; ++i)
    if (std::abs(output[i] - (input[i] * 41.0f + 1.0f)) > 1e-5f) return 1;
  std::cout << "p041 CUDA canonical kernel passed CPU oracle comparison\n";
}
