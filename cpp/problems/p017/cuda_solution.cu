...
__global__ void multi_head_attention_kernel(const float* query,
                                            const float* keys,
                                            const float* values,
                                            float* output,
                                            int query_length,
  ...
  for (int k = 0; k < key_length; ++k) {
    ...
    for (int d = 0; d < head_dim; ++d)
      score += query[(q * heads + h) * head_dim + d] * keys[(k * heads + h) * head_dim + d];
    max_score = fmaxf(max_score, score * scale);
  }

  float denominator = 0.0f;
  for (int d = 0; d < head_dim; ++d) output[(q * heads + h) * head_dim + d] = 0.0f;
  ...
  for (int k = 0; k < key_length; ++k) {
    ...
    denominator += weight;
    for (int d = 0; d < head_dim; ++d)
      output[(q * heads + h) * head_dim + d] += weight * values[(k * heads + h) * head_dim + d];
  }

  const float inv = 1.0f / denominator;
  for (int d = 0; d < head_dim; ++d) output[(q * heads + h) * head_dim + d] *= inv;
}
...
std::vector<float> cpu_reference(const std::vector<float>& query,
                                 ...
                                 int query_offset,
                                 int key_offset) {
  std::vector<float> output(query_length * heads * head_dim, 0.0f);
  ...
  for (int q = 0; q < query_length; ++q) {
    ...
    for (int h = 0; h < heads; ++h) {
      ...
      for (int k = 0; k < key_length; ++k) {
        ...
        double score = 0.0;
        for (int d = 0; d < head_dim; ++d)
          score += static_cast<double>(query[(q * heads + h) * head_dim + d]) *
                   keys[(k * heads + h) * head_dim + d];
      ...
      for (std::size_t i = 0; i < visible_keys.size(); ++i) {
        ...
        for (int d = 0; d < head_dim; ++d)
          output[(q * heads + h) * head_dim + d] +=
              static_cast<float>(probability * values[(k * heads + h) * head_dim + d]);
      }
    }
  }
...
int main() {
  ...
  multi_head_attention_kernel<<<(query_length * heads + kThreads - 1) / kThreads, kThreads>>>(
      d_q, d_k, d_v, d_o, query_length, key_length, heads, head_dim, 1, 0);
  ...
  std::cout << "p017 CUDA canonical solution passed multi-head attention validation\n";
  return 0;
}
