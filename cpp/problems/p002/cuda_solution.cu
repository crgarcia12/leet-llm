...
__global__ void p002_lesson_kernel(const float* input, float* output, int count) {
...
int main() {
  ...
  p002_lesson_kernel<<<(count + 127) / 128, 128>>>(device_input, device_output, count);
  ...
  for (int i = 0; i < count; ++i)
    if (std::abs(output[i] - (input[i] * 2.0f + 1.0f)) > 1e-5f) return 1;
  std::cout << "p002 CUDA canonical kernel passed CPU oracle comparison\n";
}
