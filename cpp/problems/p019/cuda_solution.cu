...
__global__ void online_attention_kernel(const float* query,
                                        const float* keys,
                                        const float* values,
                                        float* output,
                                        int query_length,
...
int main() {
  ...
  const std::vector<float> query{
      1.0f, 0.5f, -1.0f, -0.5f, 1.0f, 0.25f,
      2.0f, -1.5f, 0.0f, 0.25f, 0.75f, -0.5f};
  const std::vector<float> keys{
      1000.0f, -1000.0f, 0.5f,
      999.0f, -1001.0f, -0.25f,
      1002.0f, -998.0f, 0.75f,
      998.0f, -1002.0f, -0.5f,
      1001.5f, -999.5f, 0.25f};
  const std::vector<float> values{
      1.0f, 2.0f, 3.0f,
      3.0f, -2.0f, 1.0f,
      -1.0f, 0.5f, 4.0f,
  ...
  online_attention_kernel<<<(query_length * query_heads + kThreads - 1) / kThreads, kThreads>>>(
  ...
  std::cout << "p019 CUDA canonical solution passed online-attention recurrence validation\n";
  return 0;
}
